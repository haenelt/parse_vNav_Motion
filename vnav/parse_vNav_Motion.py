...
def readRotAndTrans(paths):
    ...
    ds = sorted(
        [pydicom.dcmread(x) for x in files], key=lambda dcm: dcm.AcquisitionNumber
    )
    ...
    imageComments = [
        str.split(x.ImageComments)
        for x in ds
        if "ImageComments" in x and str.split(x.ImageComments)[0] == "R:"
    ]
...
def rotationMatrixToQuaternion(m):
    # Dylan Dec 19/12
    # This algorithm taken from http://www.euclideanspace.com/maths/geometry/
    ...
    # The algorithm we're using now doesn't require us to pick some arbitrary epsilon,
    # so I like it better.
...
def diffTransformToMaxMotion(t, radius):
    ...
    return np.sqrt(
        (t_rotmax * t_rotmax)
        + (2.0 * t_rotmax) * np.linalg.norm(trans - (np.dot(trans, axis) * axis))
...
def parseMotion(rotAndTrans, tr, radius):
    ...
    diffTransforms = [
        ts[1] * np.linalg.inv(ts[0]) for ts in zip(transforms[0:], transforms[1:])
    ]
...
def parse_vnav(
    ...
    Args:
        dir_in: Path that contains a list of DICOMS that make up the vNav series.
        dir_out: Path where to save a motion_parameters_vnav.h5 file containing the
    ...
    print(f"Number of motion estimates: {len(transforms)}")
    ...
    # output to STDOUT
    print(f"mean-rms: {scores['mean_rms']}")  # time-averaged RMS motion
    print(f"mean-max: {scores['mean_max']}")  # time-averaged max motion.
    # print('\n'.join(map(str, scores['rms_scores'])))  # RMS motion over time.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse DICOM files from a vNav series and convert them into "
        "different motion scores."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=float,
        help="Repetition Time (TR) of the parent sequence (i.e., the MPRAGE) expressed "
        "in seconds.",
    )
    parser.add_argument(
        "--input",
        ...
        help="A list of DICOM files that make up the vNav series (in chronological "
        "order)",
    )
    parser.add_argument(
        "--radius",
    ...
    output_type.add_argument(
        "--mean-max", action="store_true", help="Print time-averaged max motion."
    )
    output_type.add_argument(
        "--rms-scores",
        action="store_true",
    ...
    output_type.add_argument(
        "--max-scores", action="store_true", help="Print max motion over time."
    )
    parser.add_argument(
        "--output-mats-path",
        type=os.path.abspath,
        help="path where to save a motion_parameters_vnav.h5 file containing the "
        "transformation matrices",
    )
